import { backgroundObj } from './background';
import { personObj } from './person';
import { obstacleObj, obstacleMonitor } from './obstacle';
import { dataObj } from './data';
import { scoreObj1 } from './score1';
...
export var canvasW = document.documentElement.clientWidth;
export var canvasH = document.documentElement.clientHeight;
export var space; //左右间距
var background; //游戏背景
export var person; //人物
export var bodyTail = [];

var gameTimer; //游戏定时器
export var obstacle; //障碍物
var obstacleTimer; //障碍物检测定时器
export var gifList = []; //花草
export var strList = []; //石头，障碍
export var data;
export var score1; //分数增加、减少动画
...
function init() {
    ...
    for (var i = 0; i < 2; i++) {
        ...
        bodyTail[i].src = require("../imgs/person" + i + ".png");
    }
    //障碍物
    ...
    for (var j = 0; j < 3; j++) {
        gifList[j] = new Image();
        strList[j] = new Image();
        gifList[j].src = require('../imgs/flower' + j + '.png');
        strList[j].src = require('../imgs/stone' + j + '.png');
    }
    data = new dataObj();
    data.init();
...
// gif music
export function gifMusic() {
    var gifM = document.getElementById('gift');

    gifM.play();
}
// obs music
export function obsMusic() {
    var obsM = document.getElementById('obsc');
    obsM.play();
}
